...
public:
    constexpr slot_state() noexcept
        : m_index(0), m_connected(true), m_blocked(false)
    {
    }
...
public:
    ...
    constexpr slot(cleanable &c, F &&f)
        : slot_base<Args...>(c), func{std::forward<F>(f)} {}
...
public:
    ...
    constexpr slot_pmf(cleanable &c, F &&f, P &&p)
        : slot_base<Args...>(c), pmf{std::forward<F>(f)}, ptr{std::forward<P>(p)} {}
...
public:
    ...
    constexpr slot_tracked(cleanable &c, F &&f, P &&p)
        : slot_base<Args...>(c), func{std::forward<F>(f)}, ptr{std::forward<P>(p)}
    {
    }
